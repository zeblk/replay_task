...
class StructureLearning:
    ...
    def run(self):
        ...
        def screen2():
            visual.TextStim(self.win, text='You will see the new pictures in a scrambled order, three times in a row.', height=0.1, pos=(0,.5)).draw()
            visual.TextStim(self.win, text='Then, we will ask quiz questions about the true (unscrambled) order.', height=0.1, pos=(0,.05)).draw()
            visual.TextStim(self.win, text='This process will repeat four times.', height=0.1, pos=(0,-.5)).draw()
        ...
        for run in range(4):
            for repeat in range(3):
                visual.TextStim(self.win, text='Press space when you\'re ready to see the scrambled sequences.', 
                                height=0.1, pos=(0,0)).draw()
                self.win.flip()
                key_data = event.waitKeys(keyList=["space"])
                key = key_data[0]
